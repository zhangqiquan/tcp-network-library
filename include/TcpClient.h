...
namespace zhanshop {
    ...
    protected:
        ...
        // 主线程启动 停止 2
        function<void (EventLoop*, vector<EventLoop*>)> _startCallback;
        function<void (EventLoop*, vector<EventLoop*>)> _stopCallback;
        // 子线程启动停止 2
    ...
    public:
        TcpClient(EventLoop* eventLoop, InetAddress inetAddress, unsigned int numThreads, int keepidle = 5);
//        void setTcpClientStartCallback(TcpClientEventCallback& cb);
//        void setTcpClientStopCallback(TcpClientEventCallback& cb);

        void setStartCallback(eventLoopEventCallback cb);
        void setStopCallback(eventLoopEventCallback cb);
